...
void Menus::mostrarMenu(const std::vector<MenuItem>& items) {
    ...
    do {
        ...
        switch (key) {
        case 14: // Mueve la selección en el menú hacia arriba
            if (opy > 0) {
                opy -= 2;
            }
            break;
        case 15: // Mueve la selección en el menú hacia abajo
...
void Menus::menuEstudiante() {
    std::vector<MenuItem> items = {
        {"1. VER NOTAS          ", []() { /* Acción para ver notas */ }},
        {"2. VER MATERIAS       ", []() { /* Acción para ver materias */ }},
    };
    mostrarMenu(items);
}
...
void Menus::menuVarios() {
    // Declarar los managers aquí
    ArchivoManager<Director> managerDirector("directores.dat");
    ArchivoManager<Docente> managerDocente("docentes.dat");
    ArchivoManager<Estudiante> managerEstudiante("estudiantes.dat");
...
template <class T>
void Menus::realizarAccion(ArchivoManager<T>& manager, int opcion) {
    T obj;
    // Aquí debes obtener los datos del usuario para crear un nuevo objeto `obj`
    if (manager.guardarRegistro(obj)) {
